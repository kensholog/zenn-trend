...
1. [Zenn の新興トピックに早く書いても得はない ── 9.8 万記事で先行者利得を測ったら、効いたのは経路と読者だった](https://zenn.dev/kensholog/articles/zenn-first-mover-advantage)（2026-09-05 公開）
2. [Zenn で当たる記事は 1 本目が一番多い ── 当たっても次は楽にならず、ヒットの 6 割は外から来ていた](https://zenn.dev/kensholog/articles/zenn-first-hit-and-carryover)（2026-09-05 公開）

`own_metrics/` は、この分析の結論を自分のアカウントで試すための記録です（Actions が 30 分ごとに、公開されている いいね・ブックマーク・はてブ件数を追記）。

状態: **フェーズ 1〜2 通過（2026-09-04）。結論: 新興トピックに早く書くことの利得は検出できない（早期 ÷ 後期の到達率比 1.40 に対し、成熟トピックの同じ暦月でも 1.44）。効いているのは著者の「実績」（他の記事の平均いいね）で、その中身は配信経路と既存の読者。初投稿でも Publication 所属なら 35% がいいね 10 に届き、個人・フォロワー 0 なら 2%。記事 1 は 2026-09-05 公開。第 2 プロジェクト（著者 17,943 人の記事 192,238 本の順番と、はてブ件数 66,981 本）も同日に判定: 当たる記事は 1 本目が最も多く（初投稿の 20.8%、以後単調減）、当たっても次は楽にならず（プラセボで因果は検出できず）、新参のヒットの 62% にはてブが付いていた（[decisions/0005](docs/decisions/0005-q1-q2-verdict.md)）。次は 4 週間後の再取得（2b）。C（トレンド RSS の保存）は 30 分ごとの設定で稼働中（実測は 1 日 6〜7 回）。C の問いと判定基準は 2026-09-21 に [decisions/0006](docs/decisions/0006-c-trend-entry-registration.md) で登録し、判定は 2026-10-02 以降。**
...
分析の設計案・コード・データ取得・集計・図・記事の下書きには Claude Code（Anthropic）を使っています。問いと判定基準の決定、公開の判断、内容の最終確認は筆者（kensholog）が行い、判定基準はデータを見る前にコミットで固定しています（`docs/decisions/`）。 数値と事実には出典 URL と確認日を付け、未確認のものは「未確認」と書いています。
